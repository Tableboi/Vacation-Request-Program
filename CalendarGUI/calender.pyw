...
import pandas as pd
import pyodbc
from PyQt5.QtCore import QDate, QDateTime, QRect, Qt, QTimer, pyqtSignal
from PyQt5.QtGui import QBrush, QColor, QFont, QPainter, QPen, QTextCharFormat
from PyQt5.QtWidgets import (QApplication, QCalendarWidget, QDockWidget,
...
class Appointments():
    def appointments(self):
        ...
        data = cursor.execute (" SELECT [dDateStart], [dDateEnd] FROM [PulseCoreTest5].[dbo].[PC_VacationsRequests]")       
        request = cursor.fetchone()
        dDateStart = request[0]
        dDateEnd = request[1]
        
        for d in data:
            if dDateStart.strftime("%Y-%m-%d") < today:
                pass
            else:
                datestart, dateend = (
                    dDateStart.strftime("%Y-%m-%d"),
                    dDateEnd.strftime("%Y-%m-%d"),
                )
                appts.append([datestart, dateend])
        return appts
        print(appts)
...
class Example(QMainWindow):
    ...
    def __init__(self, parent=None):
       ...
       # Physical siye and location of GUI ----
        self.left   = 1138  
        self.top    = 30
        self.width  = 302
        self.height = 445
    ...
    def showDate(self, date):
        ...
        if str(date) < str(datetime.today()):
            pass
        else:
